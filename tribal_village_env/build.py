...
def _nimby_path_args(project_root: Path) -> list[str]:
    """Return dependency paths from the config generated by ``nimby sync``."""

    nim_cfg = project_root / "nim.cfg"
    if not nim_cfg.exists():
        raise RuntimeError(f"nimby did not generate {nim_cfg}")

    path_args = [
        line.strip().replace('"', "")
        for line in nim_cfg.read_text().splitlines()
        if line.strip().startswith("--path:")
    ]
    if not path_args:
        raise RuntimeError(f"nimby generated no dependency paths in {nim_cfg}")
    return path_args
